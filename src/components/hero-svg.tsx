...
export function HeroSvg() {
	...
	const popInAnimation = (index: number) => ({
		initial: {
			scale: 0.3,
			opacity: 0,
		},
	...
	const viewSize = 800
	const radius = 200
	const pointOnCircle = (degrees = 0) =>
		`translate(${
			viewSize / 2 + radius * Math.cos((degrees * Math.PI) / 180)
		}, ${viewSize / 2 + radius * Math.sin((degrees * Math.PI) / 180)})`
	return (
		<svg preserveAspectRatio="none" viewBox={`0 0 ${viewSize} ${viewSize}`}>
						...
						<g transform={pointOnCircle(310)}>
							<g transform={`translate(-147, -40)`}>
								...
								<text
									...
									y="34"
									textLength="233"
									style={{ fill: '#FFEBEB', fontSize: 24, fontWeight: 600 }}
								>
									Make 2025 the year
								</text>
								<text
