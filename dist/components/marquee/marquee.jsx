...
export function Marquee({ children }) {
    ...
    const multiplyChildren = useCallback((multiplier) => {
        const arraySize = multiplier >= 0 ? multiplier : 0;
        return Array.from({ length: arraySize }).map((_, i) => (<Fragment key={i}>{children}</Fragment>));
    }, [children]);
    if (!isMounted) {
        return null;
    }
    return (<div className="flex flex-shrink-0 flex-grow-0 basis-auto overflow-x-hidden w-full" ref={containerRef}>
