...
import { ArticleDataSchema, MarkdownDataSchema, } from '@/utils/post-schema';
import { YamlGlobalSchema } from '@/utils/yaml-schema';
...
const parseMarkdown = async (content, slug) => {
    const { file, matter } = await parseMarkdownPage(content, slug);
    let frontMatter;
    // TODO: move this out of the database adapter
    if (slug[0] === 'help') {
        frontMatter = ArticleDataSchema.parse(Object.assign({ _contentHtml: file.value, slug, authorAvatarUrl: getAvatarImageUrl(matter.data.authorEmail) }, matter.data));
    }
    else {
        frontMatter = MarkdownDataSchema.parse(Object.assign({ _contentHtml: file.value, slug, authorAvatarUrl: getAvatarImageUrl(matter.data.authorEmail) }, matter.data));
    }
    return {
        frontMatter,
        nodes: file.result,
...
const createMarkdownData = (data) => ({
    type: 'md',
    data,
});
const createYamlData = (data) => ({
    type: 'yaml',
    data,
});
...
export const listPages = async (directory) => {
    const dbPath = env('DB_PATH');
    if (!dbPath) {
        return [null, new EnvVarError('DB_PATH')];
    }
...
export const saveFile = async (filePath, contents) => {
    const dbPath = env('DB_PATH');
    if (!dbPath) {
        return [null, new EnvVarError('DB_PATH')];
    }
...
export const parseFile = async (filePath, contents) => {
    const dbPath = env('DB_PATH');
    if (!dbPath) {
        return [null, new EnvVarError('DB_PATH')];
    }
